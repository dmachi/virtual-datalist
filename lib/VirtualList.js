define(["dojo", "dijit" , "./ComponentManagerMixin", "./Component","dojo.cache", "dijit.layout._LayoutWidget"], function(dojo, dijit,ComponentManagerMixin, Component ){

return dojo.declare([dijit.layout._LayoutWidget, ComponentManagerMixin], {
	...
	templateString: dojo.cache("virtual-datalist/resources", "templates/VirtualList.html"),
	...
	layout: function(){
		...
		return dojo.when(this.createRowComponents(), dojo.hitch(this, function(){
				if (!this._started){return}
				return dojo.when(this.render(),dojo.hitch(this, function(){	
					var headerHeight = 0;
					//console.log("createRowComponetnts complete");
	...
	createRowComponents: function(){
		...
		// to calculate how many rows to create initially.  layout() calls this function which returns a deferred.
		// This function checks to see if we have the correct number of visible children, and if so resolves the
		// deferred.  If it doesn't, it attempts to calculate the appropriate number of children, and then
		// rejects the deferred returning a true boolean.  This forces layout to be rerun.  In most cases,
		// the second pass through after adding children will simple resolve the deferred.  This function also 
		// removes extra rows if the number of visible rows changes. If the viewport size changes, children will
		// be added or removed.

		var def = new dojo.Deferred();

		//force ie to layout so we can check the height
		//TODO some has.js check?
		if (dojo.isIE){
			var j = this.domNode.offsetHeight;	
		}
		...
		if ((currentRowHeight * this._visibleRows||0+1)<=this._contentBox.h-headerHeight + (currentRowHeight*2)){
			...
			//we added some children, reject the promise so that layout() will run again
	...
	getDataPage: function(start, count){
		...
		if (this.items) {
			return this.items.slice(Math.min(start, rowCount), Math.min(start+count, rowCount));
		}
		return [];
	},
	...
	updateChildren: function(items){
		...
		//console.log(this.id,'updateChildren: ',arguments);
	...
	render: function(){
		...
		return dojo.when(dojo.when(dojo.when(this.getDataPage(this.topIndex, this._visibleRows),dojo.hitch(this,"updateChildren")),dojo.hitch(this,function(){
			//console.log("setContentHeight");
			var fc = this.getFirstChild();
			if (fc){
				this.publishInternalEvent("onSetColumnWidths", fc.getColumnWidths());
			}
			//console.log("RowHeight: ", this.rowHeight, "Row Count: ", this.rowCount);
			this.publishInternalEvent("onSetContentHeight", this.rowHeight * this.rowCount);
		})), dojo.hitch(this, function(){
			//console.log("publish onEndRender");
	...
	_setTopIndexAttr: function(index){
		...
		if (this.topIndex != index){
			this.topIndex = index;
			return dojo.when(this.render(), dojo.hitch(this, "onSetTopIndex", index));
		}
